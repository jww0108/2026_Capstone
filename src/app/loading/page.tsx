        ...
        <div className="mx-auto max-w-[1600px] px-12 py-6">
          <Card className="glass-card p-12">
            ...
            <div className="mt-8 grid grid-cols-3 rounded-2xl bg-slate-50 p-6 ring-1 ring-slate-100">
              ...
              <InfoBlock image="/images/repository_logo.png" label="분석 레포지토리" value={`${report.applicant.repoCount}개`} />
              <InfoBlock image="/images/experience_logo.png" label="지원자 경력" value={report.applicant.career} />
            </div>
            ...
            <div className="mt-7 flex items-center justify-between rounded-2xl border p-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <Link href="/help"><Button variant="secondary" className="font-black"><HelpCircle className="h-5 w-5" />문제 해결 가이드 보기</Button></Link>
                <Link href="/result"><Button className="font-black">{isComplete ? "결과 확인하기" : "데모 결과 보기"}</Button></Link>
              </div>
            </div>
          </Card>
...
function StepItem({ index, title, description, status }: { index: number; title: string; description: string; status: StepStatus }) {
  const isDone = status === "done"
  const isActive = status === "active"
  return (
    <div className={`flex items-center gap-5 border-l-2 py-4 pl-5 transition-all duration-300 ${isDone ? "border-emerald-400" : isActive ? "rounded-xl border-blue-500 bg-blue-50/70" : "border-slate-200"}`}>
      <div className={`grid h-8 w-8 -translate-x-[37px] place-items-center rounded-full text-sm font-black transition-all duration-300 ${isDone ? "bg-emerald-500 text-white" : isActive ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-600"}`}>
        {isDone ? <Check className="h-5 w-5" /> : index + 1}
      </div>
      <div className="-ml-8 flex-1">
        <h3 className={`text-xl font-black ${isActive ? "text-blue-700" : "text-slate-950"}`}>{index + 1}. {title}</h3>
      ...
      <div className="flex min-w-[110px] items-center justify-end gap-3">
        ...
        {isActive && <><span className="rounded-full bg-blue-100 px-4 py-1 text-sm font-black text-blue-700">진행 중</span><Loader2 className="h-5 w-5 animate-spin text-blue-600" /></>}
        {status === "waiting" && <span className="rounded-full bg-slate-100 px-4 py-1 text-sm font-black text-slate-600">대기</span>}
      </div>
    </div>
  )
}
