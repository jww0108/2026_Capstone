...
def _print_repo_card(diag: dict, idx: int) -> None:
    ...
    if repo_type == "personal":
        ...
        good_extras = [
            ...
            if key in EXTRA_LABELS and (
                item.get("status") in ("양호", "규칙적")
                or (key == "growth_signal" and item.get("status") in ("성장 신호", "유지"))
            )
        ]
        if good_extras:
            print("\n  ─ 추가 강점 (참고) ─")
...
def _build_github_score_dict(profile: dict) -> dict:
    return {
        ...
        "axes": list(profile.get("score_axes") or []),
        "score_detail": dict(profile.get("score_detail") or {}),
    }
...
def _build_level_dict(diag_bundle: dict, detected_domains_merged: list[str]) -> dict:
    level_info = diag_bundle.get("expected_level") or {}
    grade = level_info.get("level", "Entry")
    primary_domain = detected_domains_merged[0] if detected_domains_merged else None
    tier_label_map = {
        "Top": "Top GitHub Portfolio",
        "Competitive": "Competitive GitHub Portfolio",
        "Competitive (개인)": "Competitive (개인) GitHub Portfolio",
        "Entry": "Entry GitHub Portfolio",
    }
    tier_label = tier_label_map.get(grade, f"{grade} GitHub Portfolio")
    return {
        "grade": grade,
        "description": level_info.get("summary", ""),
        "tier_label": tier_label,
        "tier_context": "공개 GitHub 포트폴리오 성숙도 기준",
        "primary_domain_tier": {
            "domain": primary_domain,
            "grade": grade,
            "label": f"{primary_domain or '일반'} GitHub Portfolio Tier",
        },
        "caveat": (
            "개발자 역량 전체 판정이 아니라 공개 GitHub 활동과 "
            "포트폴리오 구성 신호 기준입니다."
        ),
    }
...
def _print_score_detail_block(score_detail: dict) -> None:
    """CLI: 60+30+10 점수 상세 블록."""
    axes = score_detail.get("axes") or []
    if not axes:
        return
    print("\n[점수 해석 가이드]")
    print("  이 점수는 개발자 판정이 아니라 GitHub 활동 해석/진단 지표입니다.")
    print("  커밋 항목은 활동량에 커밋 메시지 품질 계수(commit_quality_factor)를 반영합니다.")
    for axis in axes:
        label = axis.get("label", axis.get("key", "axis"))
        score = float(axis.get("score") or 0.0)
        max_score = float(axis.get("max_score") or 0.0)
        gain = float(axis.get("potential_gain") or 0.0)
        print(f"  - {label}: {score:.1f} / {max_score:.1f} (개선 여지 {gain:.1f})")
        for item in axis.get("items") or []:
            item_label = item.get("label", item.get("key", "item"))
            item_score = float(item.get("score") or 0.0)
            item_max = float(item.get("max_score") or 0.0)
            item_gain = float(item.get("potential_gain") or 0.0)
            print(
                f"      · {item_label}: {item_score:.1f} / {item_max:.1f}"
                f" (개선 여지 {item_gain:.1f})"
            )
    total_gain = float(score_detail.get("potential_gain_total") or 0.0)
    print(f"  총 개선 여지(이론치): {total_gain:.1f}점")
    ...
    async def analyze(
        ...
        try:
            ...
            return {
                ...
                "level": _build_level_dict(diag_bundle, detected_domains_merged),
...
def _print_full_report(
    ...
    _print_score_detail_block(profile.get("score_detail") or {})
