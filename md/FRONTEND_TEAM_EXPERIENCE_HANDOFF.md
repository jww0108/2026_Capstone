...
## 9) 도메인 노출 정책 (v7.2-upgrade.5)

도메인 과검출 완화 정책이 추가되었다.

- 백엔드는 `모바일 앱`/`게임 개발`을 키워드만으로 쉽게 생성하지 않도록 강증거 게이트를 사용한다.
- `detected_domains`에는 저신뢰 도메인이 남을 수 있지만,
  primary 선정/혼합판정에서는 후순위 처리될 수 있다.

프론트 권장:
- 상단 대표 도메인 표시는 `job_matching.primary_domain` 우선.
- `job_matching.detected_domains` 전체 목록은 "감지 도메인(참고)" 섹션으로 보조 노출.
- 모바일/게임이 목록 하위에만 있고 매칭 category와 불일치할 때는 강조도 낮춤(secondary badge).
